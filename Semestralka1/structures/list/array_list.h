...
namespace structures
{
	...
	inline ArrayList<T>::ArrayList():
		array_(new Array<T>(ARRAYLIST_ARRAY_INITIAL_SIZE)),
		size_(0)
	{
	}
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "Invalid ArrayList index!");
		return (*array_)[index];
	}
	...
	{
		if (array_->size() == size_)	// Nemam uz miesto
		{
			enlarge();
		}
	...
	{
		DSRoutines::rangeCheckExcept(index, size_ + 1, "Invalid ArrayList index!");
		if (index == size_)
		{
			add(data);
		}
	...
	{
		Array<T> * nove = new Array<T>(size_ + ARRAYLIST_ARRAY_ENLARGE_VALUE);
	...
	{
		ArrayListIterator otherALI = dynamic_cast<const ArrayListIterator&>(other);
	...
	{
		//ArrayListIterator otherALI = dynamic_cast<const ArrayListIterator&>(other);
		//return position_ != otherALI || arrayList_ != otherALI.arrayList_;
