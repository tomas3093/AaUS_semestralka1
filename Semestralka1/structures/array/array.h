...
namespace structures 
{
	...
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		if (this != &other)
		{
			...
			{
				throw std::invalid_argument("Arrays' sizes are different!");
			}
	...
	T& Array<T>::operator[](const int index)
	{
		...
		T * typeVectPointer = reinterpret_cast<T*>(vectorPointer);

		return *typeVectPointer;
	}
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "Invalid index!");	// Skontroluje ci je platny index
